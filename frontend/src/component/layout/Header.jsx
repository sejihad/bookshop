    ...
    <>
      <nav className="w-full shadow-md bg-white sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto px-2 py-4 flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center gap-6 text-sm font-semibold text-gray-700">
            ...
            <Link
              to="/packages"
              className={`hover:text-purple-600 transition ${
                isActive("/packages") ? "text-purple-600" : ""
              }`}
            >
              PACKAGES
            </Link>
            <Link
              to="/author"
              className={`hover:text-purple-600 transition ${
                isActive("/author") ? "text-purple-600" : ""
              }`}
            >
              AUTHOR
            </Link>
            ...
            <Link
              to="/blogs"
              className={`hover:text-purple-600 transition ${
                isActive("/blogs") ? "text-purple-600" : ""
              }`}
            >
              BLOGS
            </Link>
          ...
          <div className="flex items-center gap-4 text-gray-700 text-xl relative">
            <div className="relative">
              <Link to="/cart">
                <FiShoppingCart
                  className={`hover:text-purple-600 transition cursor-pointer text-2xl ${
                    isActive("/cart") ? "text-purple-600" : ""
                  }`}
                />
                {cartCount > 0 && (
            ...
            <button
              ...
              className="hidden md:inline cursor-pointer hover:text-purple-600 transition"
            >
              <FiSearch />
            </button>
            ...
            <div ref={userMenuRef} className="relative">
              {isAuthenticated ? (
                <img
                  src={user?.avatar?.url}
                  alt="User Avatar"
                  className={`w-8 h-8 rounded-full cursor-pointer border hover:border-purple-600 transition ${
                    isActive("/profile") ||
                    isActive("/orders") ||
                ...
                <FiUser
                  className={`cursor-pointer hover:text-purple-600 transition ${
                    isActive("/login") ? "text-purple-600" : ""
                  }`}
                  onClick={() => {
                    if (!isAuthenticated) {
                      navigate("/login");
                ...
                <div className="absolute right-0 mt-2 w-44 bg-white border shadow-xl rounded-lg z-50">
                  {isAuthenticated ? (
                    <div className="flex flex-col">
                      {user?.role === "admin" && (
                        <Link
                          ...
                          className={`flex items-center gap-2 px-4 py-2 text-sm font-medium hover:bg-gray-100 hover:text-purple-700 transition-colors duration-200 rounded-md ${
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 w-full text-left px-4 py-2 text-sm font-medium text-gray-700 hover:bg-purple-100 hover:text-purple-700 transition-colors duration-200 rounded-md"
                    >
                      <FaSignInAlt /> Login
                    </button>
                  )}
            ...
            <FiMenu
              ...
              className="md:hidden cursor-pointer hover:text-purple-600 transition"
            />
          </div>
        </div>
            ...
            >
              ...
              <form onSubmit={handleSearch}>
                <div className="relative">
                  ...
                  <button
                    type="submit"
                    className="absolute right-2 top-1/2 transform -translate-y-1/2 cursor-pointer text-gray-500 hover:text-purple-600"
                  >
                    <FiSearch />
                  </button>
                </div>
        ...
        {menuOpen && (
          <div className="md:hidden bg-white px-4 pb-4 pt-2 text-gray-700 text-sm font-semibold shadow-inner space-y-2">
            ...
            <div>
              <button
                className={`flex justify-between w-full hover:text-purple-600 ${
                  location.pathname.startsWith("/books/")
                    ? "text-purple-600"
                    : ""
                }`}
                ...
                <div className="pl-4 mt-2 space-y-1 text-gray-600">
                  {categories?.map((cat) => (
                    <Link
                      ...
                      to={`/books/${cat.slug}`}
                      className={`block hover:text-purple-600 ${
                        isActive(`/books/${cat.slug}`) ? "text-purple-600" : ""
                      }`}
                      onClick={() => setMenuOpen(false)}
                    >
                      {cat.name}
                  ...
                  <Link
                    to={`/ebook`}
                    className={`block hover:text-purple-600 ${
                      isActive(`/ebook`) ? "text-purple-600" : ""
                    }`}
                    onClick={() => setMenuOpen(false)}
                  >
                    {"EBOOKS"}
                  ...
                  <Link
                    to={`/audiobook`}
                    className={`block hover:text-purple-600 ${
                      isActive(`/audiobook`) ? "text-purple-600" : ""
                    }`}
                    onClick={() => setMenuOpen(false)}
                  >
                    {"AUDIO BOOKS"}
            ...
            <div className="pt-2">
              <button
                ...
                className="flex items-center gap-2 w-full text-left py-2 hover:text-purple-600"
              >
                <FiSearch /> Search
              </button>
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-50 bg-[#222222] flex justify-around items-center py-2 lg:hidden shadow-md">
        ...
        <Link
          to="/shop"
          className={`flex flex-col items-center ${
            isActive("/shop") ? "text-purple-500" : "text-white"
          }`}
        >
        ...
        <Link
          to="/ebook"
          className={`flex flex-col items-center ${
            isActive("/ebook") ? "text-purple-500" : "text-white"
          }`}
        >
        ...
        <Link
          to="/packages"
          className={`flex flex-col items-center ${
            isActive("/packages") ? "text-purple-500" : "text-white"
          }`}
        >
